...
$year = $_POST['year'];
$date = $year."-".$month."-".$day;
$query= "INSERT INTO Token(boy_id,company_id,points,token_date) VALUES ('$boy_id','$company_id','$points','$date')";
try{
    $check = mysql_query("SELECT p_iva FROM Company WHERE p_iva='$company_id'", $conn);
    $rows = mysql_num_rows($check);
    if($rows==0)
    {
        echo $lang['COMPANY_DONOT_EXISTS'];
